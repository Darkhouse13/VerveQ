...
function randomRun(b: BoardSpec, cfg: EngineConfig, rngSeed: string): RunResult {
  ...
  while (state.phase !== "done") {
    const choice: Choice =
      state.phase === "draft"
        ? { type: "pick", offerIndex: rngInt(rng, cfg.offersPerRow) }
        : { type: rng() < 0.5 ? "push" : "bank" };
    state = applyChoice(b, cfg, state, choice);
  }
...
describe("replay identity (seed + choiceLog => identical result)", () => {
  it("holds across 150 random runs", () => {
    for (let i = 0; i < 150; i++) {
...
describe("state serialization round-trip", () => {
  it("a JSON round-tripped mid-run state finishes identically", () => {
    for (let i = 0; i < 50; i++) {
      ...
      const nextChoice = (state: RunState): Choice =>
        state.phase === "draft"
          ? { type: "pick", offerIndex: rngInt(rng, config.offersPerRow) }
          : { type: rng() < 0.5 ? "push" : "bank" };

      // Play a random prefix, snapshot through JSON, then finish both copies
      // with the same recorded choices.
      let live = initRun(b);
      const prefixLen = 1 + rngInt(rng, config.rows);
...
describe("form is a pure function of (boardSeed, cardId, roundIndex)", () => {
  ...
  it("a shared card has identical form across two different draft paths", () => {
    ...
    // Both paths pick offer 0 in row 0, then diverge completely. F1 plays
    // automatically after the last pick, so rounds[0] exists either way.
...
describe("engine purity", () => {
  it("applyChoice does not mutate its input state", () => {
    ...
    while (state.phase !== "done") {
      ...
      const choice: Choice =
        state.phase === "draft" ? { type: "pick", offerIndex: 0 } : { type: "push" };
