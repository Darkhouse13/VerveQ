...
// Play-first flow: pick what to play, take your turn, then invite an opponent
// from the results screen. No opponent is chosen up front.
type Step = "kind" | "topic" | "difficulty";
...
export default function CreateDuelModal({
  ...
  const goBack = () => {
    ...
    if (step === "difficulty") return setStep(kind === "knowledge" ? "topic" : "kind");
    handleClose();
  };
  ...
  const handlePlay = async () => {
    ...
    try {
      ...
      // Always play-first: create an opponent-less link duel, take your turn,
      // then invite from the results screen.
