...
RULE="Host(\`${DOMAIN}\`) || Host(\`${WWW_DOMAIN}\`)"
docker run -d --name verveq-web --restart unless-stopped \
  ...
  --label traefik.http.routers.verveq-http.entryPoints=http \
  --label "traefik.http.routers.verveq-http.rule=${RULE}" \
  --label traefik.http.routers.verveq-http.middlewares=verveq-https \
  --label traefik.http.routers.verveq-https.entryPoints=https \
  --label "traefik.http.routers.verveq-https.rule=${RULE}" \
  --label traefik.http.routers.verveq-https.middlewares=verveq-gzip \
  --label traefik.http.routers.verveq-https.tls=true \
  --label traefik.http.routers.verveq-https.tls.certresolver=letsencrypt \
