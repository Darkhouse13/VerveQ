...
const ANCHOR_PRICES = [13.0, 11.5, 10.0, 8.5, 7.0, 5.5, 4.0] as const;
...
// -------------------------------------------------------------- ANCHOR_GRID.md
{
  let md = header('FW-PR1 — Anchor grid (Phase B owner pass)');
  md += `28 slots: 4 positions × 7 anchor price points. Candidates are drawn from the **top-five proxied pool only** (${data.manifest.counts.topfive} players; owner ruling — promoted cohort and flagged players are priced separately). For each slot: the player at that quantile of his position's proxy ranking, plus the two nearest alternates. Confirm or swap each slot; interpolation between confirmed anchors is Phase C.\n`;

  for (const pos of POSITIONS) {
    const pool = data.players.filter((p) => p.pool === 'topfive' && p.position === pos).sort(byProxyDesc);
    md += `\n## ${pos} (${pool.length} proxied)\n\n`;
    md += `| Price | Candidate | Proxy /90 | Minutes | Alternate A | Alternate B |\n`;
    md += `| --- | --- | --- | --- | --- | --- |\n`;
    ANCHOR_PRICES.forEach((price, i) => {
      const q = i / (ANCHOR_PRICES.length - 1);
      const idx = Math.round(q * (pool.length - 1));
      const cand = pool[idx];
      const alt = (j: number): string => {
        const p = pool[j];
        return p === undefined || p === cand ? '—' : `${p.name} (${p.clubName}) ${f2(p.proxy)}, ${p.minutes}'`;
      };
      md += `| **${price.toFixed(1)}** | **${cand.name}** (${cand.clubName}) | ${f2(cand.proxy)} | ${cand.minutes}' | ${alt(idx === 0 ? idx + 1 : idx - 1)} | ${alt(idx === 0 ? idx + 2 : idx + 1)} |\n`;
    });
  }
...
// ------------------------------------------------------------- DISTRIBUTION.md
{
  ...
  md += table(data.players.filter((p) => p.pool === 'topfive'), `Top-five pool (${data.manifest.counts.topfive}) — the anchor-grid population`);
  md += table(
