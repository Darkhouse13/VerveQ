...
> **⚠️ Partially superseded (2026-07) — Daily Survival shipped.**
> Daily Survival is LIVE and playable. Sections 1, 9, 10 and 12 below call it
> unimplemented and cite a rejection string and a function that exist NOWHERE in
> the repo (`"Daily survival is not implemented yet."`,
> `dailyChallenge.assertDailyQuizMode`); §12's `dailyChallenge.ts:31-34` citation
> points at constant declarations (`DAILY_QUIZ_COUNT`, `MAX_TIME_SEC`), not a
> mode check. Ignore those four claims. The real shape: one shared 10-round
> football-only run per UTC day, generated once onto the `dailyChallenges` row
> and consumed in order by every player — determinism comes from that frozen
> snapshot, not seeded RNG (`app/convex/survivalSessions.ts:1121-1370`,
> `DAILY_SURVIVAL_ROUNDS = 10`, `ensureDailySurvivalChallenge` at :1164,
> `startDailyGame` at :1246; pre-generated by the daily cron at
> `app/convex/dailyChallenge.ts:439`, imported at :12). The route is
> `/v2/daily-survival` (`app/src/App.tsx:504`), which renders
> `SurvivalPlayScreen` with a `daily` prop — there is deliberately no separate
> `DailySurvivalScreen.tsx`. It is username-tier, one attempt per UTC day, and
> never ranked: `app/convex/games.ts:198-201` throws "Daily survival runs are not
> ranked" for any session carrying a `dailyDate`, and the result finalizes into
> `dailyAttempts`. See `app/src/test/dailySurvivalContract.test.ts`.

> **⚠️ Superseded (2026-04) — the Python backend is gone.**
> The FastAPI surface was removed in `ad7a3d5`; there is no `backend/` directory
> in the repo. Sections 2, 3, 4, 5 and 15 quote and cite `backend/sports/*.py`,
> `backend/services/*.py`, and `backend/routes/survival/*.py` as though they
> were live paths, and §15 lists them in a "Complete File Reference" of current
> files. Those rows are dead. The Convex implementation is the only runtime; the
> Python material below is retained purely as design history for the fame
> weighting and similarity scoring that Convex did not carry over.
...
**Current playable variants:**
- **Standard Survival** — unlimited attempts, ELO-rated
- **Daily Survival** — one shared 10-round football run per UTC day, one attempt,
  never ELO-rated. Shipped 2026-07; see the banner above. *(This section
  originally listed it as "declared but unwired", rejected by a
  "Daily survival is not implemented yet." error. No such string exists.)*
...
### Python Backend (Historical / Superseded — files deleted)

These paths no longer exist. The whole `backend/` tree was removed in `ad7a3d5`
(2026-04). The table is kept as design history for the fame weighting and
similarity scoring described in sections 4 and 5, which Convex did not carry
over.
...
*(Corrected 2026-07.)* This section claimed Daily Survival was not wired into
playable runtime, citing `dailyChallenge.assertDailyQuizMode` — a function that
does not exist in the repo. Daily Survival is live; see the banner above. There
is indeed no `DailySurvivalScreen.tsx`, but that is by design: `/v2/daily-survival`
renders the shared `SurvivalPlayScreen` with a `daily` prop.
...
3. **Daily forfeit** — applies to daily attempts. *(Corrected 2026-07: this
   originally read "Daily Survival is not currently playable". It is playable —
...
*(Corrected 2026-07.)* This section claimed Daily Survival was declared in
shared types but not implemented. Every load-bearing citation in it was wrong:
`app/convex/dailyChallenge.ts:31-34` does not reject non-quiz daily modes —
those lines are the constant declarations `DAILY_QUIZ_COUNT` and `MAX_TIME_SEC`
— and `App.tsx` defines `/v2/daily` (:501) *and* `/v2/daily-survival` (:504).

Daily Survival is implemented and playable. See the banner at the top of this
document for the current shape, entry points, and contracts.
...
*(Corrected 2026-07: the five `backend/**.py` rows at the foot of this table were
listed as live paths. They were deleted in `ad7a3d5` and are marked as such. The
Daily Survival paths are missing from this table — it predates that mode; see the
...
| ~~`backend/sports/survival_engine.py`~~ | **Deleted** (`ad7a3d5`): `SurvivalEngine` class |
| ~~`backend/sports/survival_helpers.py`~~ | **Deleted** (`ad7a3d5`): `FameCalculator`, `PlayerSelector`, `PlayerInfoExtractor` |
| ~~`backend/sports/utils.py`~~ | **Deleted** (`ad7a3d5`): `normalize_name`, `calculate_similarity`, `get_player_initials` |
| ~~`backend/services/survival_session.py`~~ | **Deleted** (`ad7a3d5`): session management service |
| ~~`backend/routes/survival/session.py`~~ | **Deleted** (`ad7a3d5`): REST API endpoints |
