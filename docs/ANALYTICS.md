...
| SEO curiosity funnel | `app/src/lib/seoFunnel.ts` (started by `components/seo/SeoPageHost.tsx`) |
...
### Anonymous curiosity funnel (static SEO pages only)
...
**Since 2026-09-23 (SEO-2)** the static pages are generated by
`app/seo/build.mjs` and carry the app bundle (so Play opens the game with no
page load). The funnel therefore runs inside the app through `lib/analytics`
(same token, same localStorage identity) instead of a separate posthog-js load
from `funnel.js`; event names and properties are unchanged, so the series
continues. Two additions: a `page_type` property (`game`, `games_hub`,
`quiz_archive`, `grid_answers`, `career_path_quiz`) now that the layer has data
pages as well as game pages, and `game_start_intent.target` (the Play href).
The funnel runs on every page under `/games/`, `/football-quiz/`,
`/who-played-for/` and `/career-path-quiz/`; `entry_source` is `seo` for
visits that land there. Crawlers stay out through posthog-js's own bot filter,
as before.
...
- `opt_out_useragent_filter: true` — posthog-js silently drops everything an
  ...
  `opt_out_useragent_filter` only in test mode (`lib/analytics.ts`), false for real traffic
  on the live host.
